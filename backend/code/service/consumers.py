...
class ServiceConsumer(AsyncWebsocketConsumer):
    ...
    async def _handle__vue_cb_users_services_status(self, json_data={}):
        ...
        if not service:
            await self.send(text_data=json.dumps({
                "class": "sb.error",
                "message": "Missing service configuration for {}".format(service_name)
            }))
            return
        ...
        for user_service in service.userservice_set.all():
            ...
            data.update({
                "linked": True,
                "verified": user_service.last_verified.isoformat() if user_service.last_verified else False,
    ...
    async def _handle__vue_cb_users_services_oauth_flow(self, json_data={}):
        ...
        upstream_scope = upstream_json['scope'] if upstream_json.get('scope') else service.broad_scope
